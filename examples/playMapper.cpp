...
*/
	...
	*m << osc::EndBundle;
	...
	qpmanager::addQueue("recycleVideo3");
	qpmanager::addEmptyImages("recycleVideo2",15);
	qpmanager::addEmptyImages("recycleVideo3",15);

	qpmanager::addPlugin("F2","pluginFfmpeg");
	qpmanager::connect("recycleVideo2","F2","in");
	qpmanager::connect("display","F2","out");

	qpmanager::addPlugin("F3","pluginFfmpeg");
	qpmanager::connect("recycleVideo3","F3","in");
	qpmanager::connect("display","F3","out");
...
/*
	...
	qpmanager::connect("drip2","D","in");
	qpmanager::connect("display","D","out");

	qpmanager::addPlugin("D3","pluginDrip");
	qpmanager::connect("drip3","D","in");
	qpmanager::connect("display","D","out");
*/
	...
	*m << osc::BeginMessage("/max-fps")<< 28.0f <<osc::EndMessage;
	*m << osc::EndBundle;
	...
	*m << osc::BeginBundleImmediate;
	//*m << osc::BeginMessage("/start") << "/home/roys/Videos/VID_20131207_210313.mp4" << "" << "rgb" << "/main/B/tex" << 0 << osc::EndMessage;
	*m << osc::BeginMessage("/max-fps")<< 29.0f <<osc::EndMessage;
	*m << osc::EndBundle;
	qpmanager::add("recycleVideo3",m); // on pourrait donner "plugin","port" et utiliser ca
	...
	*m << osc::BeginBundleImmediate;
	// init les trois vues
	*m << osc::BeginMessage("/tag")<<"A"<<osc::EndMessage;
	*m << osc::BeginMessage("/tag")<<"B"<<osc::EndMessage;
	*m << osc::BeginMessage("/tag")<<"C"<<osc::EndMessage;
	*m << osc::BeginMessage("/set/view")<<"A"<<"/main/A/homog"<<osc::EndMessage;
	*m << osc::BeginMessage("/set/view")<<"B"<<"/main/B/homog"<<osc::EndMessage;
	*m << osc::BeginMessage("/set/view")<<"C"<<"/main/C/homog"<<osc::EndMessage;
	*m << osc::BeginMessage("/go")<<"A"<<osc::EndMessage; // generate first "full" view
	*m << osc::BeginMessage("/go")<<"B"<<osc::EndMessage; // generate first "full" view
	*m << osc::BeginMessage("/go")<<"C"<<osc::EndMessage; // generate first "full" view
	// on demarre avec "A"
	*m << osc::BeginMessage("/tag")<<"A"<<osc::EndMessage;
	//*m << osc::BeginMessage("/go")<<"default"<<"/deform/A/tex"<<osc::EndMessage; // to see the transform
	*m << osc::EndBundle;
	...
	*m << osc::BeginMessage("/new-quad/main/C") << 0.0f << 1.0f << 0.0f << 1.0f
	...
	*m << osc::BeginMessage("/new-quad/main/A") << 0.0f << 1.0f << 0.0f << 1.0f
        << -10.0f << NORMAL_SHADER << osc::EndMessage;
	//*m << osc::BeginMessage("/new-quad/deform/A") << 0.0f << 1.0f << 0.0f << 1.0f << -10.0f << "normal" << osc::EndMessage;
#ifdef HOMOGRAPHIE
	//
	...
	qpmanager::reservePort("V","toF3");
	qpmanager::connect("recycle","V","toHomog");
	qpmanager::connect("recycleVideo","V","toF");
	qpmanager::connect("recycleVideo2","V","toF2");
	qpmanager::connect("recycleVideo3","V","toF3");

	*m << osc::BeginMessage("/defevent-ctrl") << "Z" << "toF" << "/start" << (argc>1?argv[1]:"/home/roys/Videos/VID_20131207_205912.mp4") << "" << "rgb" << "/main/A/tex" << 0 << osc::EndMessage;
	*m << osc::BeginMessage("/defevent-ctrl") << "Z" << "toF2" << "/start" << (argc>2?argv[2]:"/home/roys/Videos/VID_20131207_205912.mp4") << "" << "rgb" << "/main/B/tex" << 0 << osc::EndMessage;
	*m << osc::BeginMessage("/defevent-ctrl") << "Z" << "toF3" << "/start" << (argc>3?argv[3]:"/home/roys/Videos/VID_20131207_205912.mp4") << "" << "rgb" << "/main/C/tex" << 0 << osc::EndMessage;

	*m << osc::BeginMessage("/defevent-ctrl") << "Z" << "toD" << "/fps" << 30.0f << osc::EndMessage;

	*m << osc::BeginMessage("/defevent-ctrl") << "X" << "toF" << "/stop" << osc::EndMessage;
	*m << osc::BeginMessage("/defevent-ctrl") << "X" << "toF2" << "/stop" << osc::EndMessage;
	*m << osc::BeginMessage("/defevent-ctrl") << "X" << "toF3" << "/stop" << osc::EndMessage;
	...
	*m << osc::BeginMessage("/defevent-ctrl") << "5" << "toHomog" << "/set/p" << 0 << osc::Symbol("$xr") << osc::Symbol("$yr") << osc::EndMessage;
	*m << osc::BeginMessage("/defevent-ctrl") << "6" << "toHomog" << "/set/p" << 1 << osc::Symbol("$xr") << osc::Symbol("$yr") << osc::EndMessage;
	*m << osc::BeginMessage("/defevent-ctrl") << "7" << "toHomog" << "/set/p" << 2 << osc::Symbol("$xr") << osc::Symbol("$yr") << osc::EndMessage;
	*m << osc::BeginMessage("/defevent-ctrl") << "8" << "toHomog" << "/set/p" << 3 << osc::Symbol("$xr") << osc::Symbol("$yr") << osc::EndMessage;
	*m << osc::BeginMessage("/defevent-ctrl") << "A" << "toHomog" << "/tag" << "A" << osc::EndMessage;
	*m << osc::BeginMessage("/defevent-ctrl") << "B" << "toHomog" << "/tag" << "B" << osc::EndMessage;
	*m << osc::BeginMessage("/defevent-ctrl") << "C" << "toHomog" << "/tag" << "C" << osc::EndMessage;
	...
	*m << osc::EndBundle;
	...
	//for(int i=1;i<argc;i++) loadImg(argv[i],"/main/A/tex","display");
		...
		loadImg(std::string(IMGV_SHARE)+"/images/indianHead.jpg","/main/C/tex","display");
		//loadImg(std::string(IMGV_SHARE)+"/images/refA.png","/main/A/tex","display");
