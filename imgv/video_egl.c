...
void VideoEGL_GetSurfaceContextDisplay( VideoEGL *ve, EGLSurface *pSurface, EGLContext *pContext, EGLDisplay *pDisplay ) {
    if( pSurface ) *pSurface = ve -> surface;
    if( pContext ) *pContext = ve -> context;
    if( pDisplay ) *pDisplay = ve -> display;
}
