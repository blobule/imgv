...
pluginWebcam::~pluginWebcam() { log << "deleted"<<warn; }

void pluginWebcam::init() {
...
void pluginWebcam::uninit() {
	log << "uninit"<<warn;
	stream.release();
}

bool pluginWebcam::loop() {
	...
	if( camnum>=0 ) {
		...
		stream.read(*i1);
		if( i1->n < 10 ) cout << "frame size is "<<i1->cols<<" x "<<i1->rows<<endl;
			double xs=stream.get(CV_CAP_PROP_FRAME_WIDTH);
			double ys=stream.get(CV_CAP_PROP_FRAME_HEIGHT);
			cout << " x="<<xs<<" y="<<ys<< endl;
...
//
bool pluginWebcam::decode(const osc::ReceivedMessage &m) {
	log << "decoding " << m << info;
    int32 k;
...
void pluginWebcam::dump(ofstream &file) {
...
void pluginWebcam::process(Mat &im) {
