...
uniform float fade;
uniform sampler2D homogL;
uniform sampler2D homogR;
uniform float fovomni;


// homog : LUT for homography mapping
// eye: 0=left, 1=right, 2=stereoLR
...
// applique une distrorsion radiale
// (assume que la texture est deja entre -1 et 1)
// 1,0.22,0.24,0 -> k0..k3
//
vec2 radialWarp(vec2 p,vec4 k)
{
...
void main(void) {
    ...
    {
      eye=0;
      h = texture2D(homogR,pos).rgb;
      sray = vec3( h.x, h.y, 1.0 );    
    }
    else
    {
      eye=1;
      h = texture2D(homogL,pos).rgb;
      sray = vec3( h.x, h.y, 1.0 );    
    }
    ...
    if (camview)
    {
      ...
      // distorsion radiale
      // sray va de 0 a 1 en x et y
      vec4 k=vec4(1.0,0.22,0.24,0.0);
      sray.xy=radialWarp(sray.xy,k);
